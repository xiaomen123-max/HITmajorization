...
class Dataset:
    ...
    def load_dataset(self):
        ...
        self.gene_go_adj = loaded_tensor['gene_go'][:half_length]
        self.dis_hpo_adj = loaded_tensor['disease_hpo'][:half_length]
        self.dis_do_adj = loaded_tensor['disease_do'][:half_length]
    ...
    def construct_hypergraph_g(self):
        ...
        self.original_hyperedges_gene_gene = copy.deepcopy(self.hyperedges_gene_gene)
    ...
    def construct_hypergraph_d(self):
        ...
        self.original_hyperedges_tbga = copy.deepcopy(self.hyperedges_tbga)
    ...
    def get_data_forwards(self):
        data = {
            ...
            'hyperedges_gene_gene': self.hyperedges_gene_gene,
        ...
        return {
            'gene_total_incidence_matrix': data['gene_total_incidence_matrix'],
            'disease_integrated_incidence': data['disease_integrated_incidence']
        }
    ...
    def _apply_augmentation(self, data):
        ...
        if random.random() < self.augment_prob:
            ...
            if augment_fn == self._random_edge_dropout:
                data['gene_total_incidence_matrix'] = augment_fn(data['gene_total_incidence_matrix'])
                data['disease_integrated_incidence'] = augment_fn(data['disease_integrated_incidence'])
            elif augment_fn == self._feature_noise_injection:
                # 这里需要获取节点特征，可能需要修改模型结构
                pass
            else:
                data['hyperedges_gene_gene'] = augment_fn(data['hyperedges_gene_gene'])
                data['hyperedges_tbga'] = augment_fn(data['hyperedges_tbga'])
        return data

    def reset_hyperedges(self):
        """重置为原始超边结构"""
        self.hyperedges_gene_gene = copy.deepcopy(self.original_hyperedges_gene_gene)
        self.hyperedges_tbga = copy.deepcopy(self.original_hyperedges_tbga)
        # 重新生成incidence matrix
        gene_integrated = hnx.Hypergraph(self.hyperedges_gene_gene)
        self.gene_integrated_incidence = gene_integrated.incidence_matrix()
        self.gene_incidence_matrix = self.gene_integrated_incidence.toarray()
